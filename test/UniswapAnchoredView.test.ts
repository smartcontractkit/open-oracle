...
async function setup({ isMockedView }: SetupOptions) {
  ...
  const tokenConfigs: TokenConfig[] = [
    {
      ...
      baseUnit: uint(1e18),
      priceSource: PriceSource.FEED,
      fixedPrice: BigNumber.from(0),
      uniswapMarket: usdcEthPair.address,
      feed: cToken.ETH.feed.address,
  ...
  if (isMockedView) {
    ...
    uniswapAnchoredView = await new UniswapAnchoredView__factory(
      deployer
    ).deploy(anchorMantissa, anchorPeriod);
  }
...
describe("UniswapAnchoredView", () => {
  ...
  describe.only("getUnderlyingPrice", () => {
    ...
    it("should return fixed ETH amount if SAI", async () => {
      const price = 3950e8;
      await cToken.ETH.feed.updateAnswer(price);
    ...
    it("should return reported ETH price", async () => {
      await cToken.ETH.feed.updateAnswer(3950e8);
    ...
    it("should return reported WBTC price", async () => {
      await cToken.ETH.feed.updateAnswer(3950e8);
      await cToken.WBTC.feed.updateAnswer(49338e8);
    ...
    it("should use known gas for fetching ETH price (old)", async () => {
      await cToken.ETH.feed.updateAnswer(3950e8);
      // priceInternal:      returns 3950e6
      // getUnderlyingPrice: 1e30 * 3950e6 / 1e18 = 3950e18
      const readTx =
        await uniswapAnchoredView.populateTransaction.getUnderlyingPrice(
          cToken.ETH.addr
        );
      const tx = await deployer.sendTransaction(readTx);
      const receipt = await tx.wait();
      expect(receipt.gasUsed).to.equal(28562);
    });

    it("should use known gas for fetching WBTC price (old)", async () => {
      await cToken.ETH.feed.updateAnswer(3950e8);
      await cToken.WBTC.feed.updateAnswer(49338e8);
      ...
      const tx = await deployer.sendTransaction(readTx);
      const receipt = await tx.wait();
      expect(receipt.gasUsed).to.equal(29002);
    });
  });
  ...
  describe("constructor", () => {
    ...
    // TODO: Replace with `addTokenConfig` test (maybe in UAVConfig tests)
    ...
    // TODO: Test in UAVConfig#addTokenConfig
